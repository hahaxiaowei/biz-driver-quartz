package com.huntkey.rx.springbootquartzmanage.base;
...
import com.huntkey.rx.springbootquartzmanage.constants.WFHandlerTypeConstants;
import com.huntkey.rx.springbootquartzmanage.constants.WorkFlowConstants;
...
import com.huntkey.rx.springbootquartzmanage.service.BizFormService;
...
public abstract class BaseOrderService<T extends OrderEntity, S extends PropertyBaseEntity> implements IBaseOrderService<T> {
    ...
    public BaseOrderService() {
        ...
        entitySetClass = (Class<S>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[1];
    }
     ...
     * @return
     */
    public abstract String getAuditVaildParamAndUpdateErrorMsg(JSONObject ordeParamObj) throws Exception;
    ...
    public Result insertData(T entity) {
        ...
        try {
            ...
            if (StringUtil.isNullOrEmpty(entity.getId())) {
                entity.setOrde_status(OrderStatus.ORDE_STATUS_1);
                ...
                param.addColumn(OrderProperty.ORDE_STATUS);
                param.setWhereExp(OrmParam.and(param.getEqualXML(NodeConstant.ID, entity.getId())));
                List<Map<String, Object>> list = ormService.selectMapList(JobbaseinfoEntity.class, param);
                String ordeStatus = null;
                if (CollectionUtils.isNotEmpty(list)) {
                    Map<String, Object> map = list.get(0);
                    ordeStatus = (String) map.get(OrderProperty.ORDE_STATUS);
                }
                ...
                entity.setOrde_status(ordeStatus);
                // 更新主表记录
                ormService.updateSelective(entity);
                // 删除原先保存的关联表记录
                OrmParam ormParam = new OrmParam();
                if (!entitySetClass.equals(PropertyBaseEntity.class)) {
...
//    @Transactional(readOnly = false, rollbackFor = Exception.class)
    public String submitFlow(String ordeId, String orderRodeObj, boolean errorRollBack) {
        try {
            ...
            String nbr = "";
            if (!getOrderNbrPrefix().equals(getOrderNbrTempPrefix())) {
                nbr = commonService.getCode(NullUtils.valueOf(getOrderNbrPrefix()));
                entity.setOrde_nbr(nbr);
            }
            entity.setOrde_status(OrderStatus.ORDE_STATUS_2);
            entity.setId(ordeId);
            ormService.updateSelective(entity);
            return nbr;
        } catch (Exception e) {
        ...
        try {
            ...
            switch (handlerType) {
                ...
                case WFHandlerTypeConstants.REVOKE: {
                    return updateOrderStatus(orderInstanceId, OrderStatus.ORDE_STATUS_1);
                }
                case WFHandlerTypeConstants.RETURN_BACK: {
                    return updateOrderStatus(orderInstanceId, OrderStatus.ORDE_STATUS_6);
                }
                default: {
                    result.setRetCode(Result.RECODE_SUCCESS);
